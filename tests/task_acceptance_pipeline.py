...
RESEARCH_TRACE = [
    {
        ...
        "explicit_premise": "For each active translation, W is the maximum natural width of General, Shortcuts, and Mouse, including the scrollbar chrome required by the 16-row table.",
    },
    {
        "hop": 2,
    ...
    },
    {
        ...
        "explicit_premise": "The Settings width remains fixed at the current language's content-derived W; only the natural per-tab height is animated.",
    },
    {
        "hop": 5,
        ...
        "explicit_premise": "The selected category and current translation are authoritative; only the category is persisted, while any legacy saved geometry is removed.",
    },
    {
        "hop": 6,
...
CASES = [
    ...
    make_case(
        "SET-005",
        "设置页使用当前语言下三个 Tab 无水平滚动所需的共同最小宽度，并禁用手动调整尺寸。",
        "unit",
        "tests/tst_qviewtests.cpp::WindowBehaviorTests::testSettingsDialogUsesFixedWidthAndTabHeights",
        "验证共同内容宽度、size grip 和最小/最大宽度。",
        ["Debug 测试二进制已构建。", "Qt Cocoa 平台插件可用。"],
        {"suite": "WindowBehaviorTests", "test": "testSettingsDialogUsesFixedWidthAndTabHeights", "width_rule": "max(natural tab widths)"},
        ["创建设置对话框。", "读取三个页面的自然宽度、共同宽度、宽度边界和 size grip。"],
        "共同宽度等于三个页面所需宽度的最大值；minimumWidth、maximumWidth 和实际宽度一致，且 size grip 不可用。",
        ["对话框关闭。"],
    ),
    make_case(
        "SET-006",
    ...
    make_case(
        ...
        "unit",
        "tests/tst_qviewtests.cpp::WindowBehaviorTests::testSettingsTabTransitionAndMouseReopen",
        "验证新实例只恢复 Tab，并按当前翻译重新推导同一稳定尺寸。",
        ["Debug 测试二进制已构建。"],
        {"suite": "WindowBehaviorTests", "test": "testSettingsTabTransitionAndMouseReopen", "tab": "Mouse"},
        ["切到 Mouse。", "关闭并销毁 QDialog 实例。", "创建新实例并比较 Tab、宽度和高度。"],
        "新实例仍为 Mouse，且宽度和高度与关闭前的稳定 Mouse 尺寸完全相同。",
        ["原始 optionstab 和 optionstabversion 恢复；遗留 optionsgeometry 不参与尺寸决定。"],
    ),
    make_case(
        "SET-010",
    ...
    make_case(
        ...
        ["Qt Linguist lupdate 可执行。", "四个 TS 文件可读。"],
        {"contexts": ["QVOptionsDialog"], "scope": "General", "explicit_entries": ["Appearance:", "Show small images at 1:1", "Associate all supported formats", "Use checkerboard background after opening image", "Open images in the same window", "Smooth scaling:", "Auto update check:"]},
...
def static_settings_contract(repo: Path, sources: dict[str, str]) -> tuple[bool, dict[str, Any]]:
    ...
    checks = {
        ...
        "content_derived_shared_width": "settingsDialogWidth" in header and "qMax(mouseNaturalWidth, shortcutsNaturalWidth)" in cpp and "settingsNaturalPageWidth" in cpp,
        "resize_disabled": "setSizeGripEnabled(false)" in cpp and "setFixedSize(settingsDialogWidth, targetHeight)" in cpp,
        "natural_general_mouse_height": "ScrollBarAlwaysOff" in cpp and "naturalLayoutSize" in cpp and "content->minimumHeight()" in cpp,
        "shortcut_rows_constant": "ShortcutsVisibleRows = 16" in header and "table->rowHeight(row)" in cpp,
        "derived_geometry_is_authoritative": 'settings.remove("optionsgeometry")' in cpp and "restoreGeometry" not in cpp,
