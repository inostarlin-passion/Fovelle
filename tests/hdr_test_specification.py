...
CASES = [
    ...
    case("ST-HDR-OFFSCREEN-PREPARATION", "HDR 昂贵首求值必须在可见渐亮前由 Core Image 管理的 float intermediate 完成并缓存。", "时间行为", "static", "既防止 RAW 首求值吞掉过渡窗口，也防止自有纹理反向导入产生未解析 GPU tile。", "renderer 源码可读。", "SDR/HDR Core Image managed intermediates 与串行预热 command buffer。", "检查 imageByInsertingIntermediate:YES、预热、prepared 合成，并确认 renderer 不调用 imageWithMTLTexture。", "过渡复用 Core Image 管理的浮点端点；应用自有 render-target 从不作为后续 CI 输入。", "缓存随图像或几何 generation 销毁。", "tests/hdr_quality_static.py"),
    case("ST-HDR-GEOMETRY-LIFECYCLE", "缩放、拖动或 resize 改变完整视口几何时，旧 Metal generation 必须失效并等待几何稳定。", "功能正确性", "static", "防止 DNG 左上角局部帧和拖动残影。", "view/renderer 源码可读。", "viewport size、四角、34ms debounce、generation 与 layer opacity。", "检查完整几何比较、单次 debounce、invalidateGeometry、proxy 显示和 autoresizingMask。", "几何变化期间只显示同步 Qt proxy；旧 drawable 不再覆盖新位置；稳定后建立新 generation。", "不触发真实输入。", "tests/hdr_quality_static.py"),
    case("ST-HDR-OBSERVABILITY", "运行态必须非侵入式输出解码、渲染、headroom、过渡与 viewport 像素裁剪遥测。", "可测试性", "static", "验证机器可观测接口与桌面无关的截图测量。", "源码可读。", "FOVELLE_HDR compact JSON。", "检查 JSON 日志、decode/render timing、headroom/transition 及 viewport global origin/size/DPR 字段。", "系统测试可从 stderr 恢复完整状态，并把截图指标限制到被测 viewport。", "默认未设置环境变量时不输出。", "tests/hdr_quality_static.py"),
    ...
    case("SYS-HDR-GAINMAP-JPEG-EDR", "真实窗口中的 gain-map JPEG 必须达到 EDR target>1。", "功能正确性", "system", "端到端验证非 RAW HDR。", "XDR Mac、Cocoa app 和样例可用。", "JPEG 启动 3 次。", "打开窗口、采集 5.2s FOVELLE_HDR telemetry。", "每次识别 adaptive HDR/gain map 且 target>1.1。", "终止测试进程，保存日志。", "tests/hdr_quality_system.py"),
    ...
    case("SYS-HDR-FINAL-LAYOUT-BEFORE-METAL", "任何 Metal 提交都必须使用稳定的完整 viewport 几何和匹配的 drawable。", "功能正确性", "system", "端到端验证 DNG 完整→左上角→完整故障修复。", "真实 Cocoa Window、DNG 与 screencapture 可用。", "三次 DNG telemetry，以及 clean-start 后 2.0～4.8s 的 5 帧截图。", "验证 layout_ready、geometry_pending=false、drawable match；计算各帧边缘向量与最终帧的余弦相似度。", "提交只发生在稳定 generation、drawable 完整匹配；5 帧结构相似度均≥0.90（修复实测最低 0.940，局部帧基线低于 0.64）。", "保留截图、哈希和结构指标并关闭窗口。", "tests/hdr_quality_system.py"),
    ...
    case("SYS-HDR-INTERACTION-GEOMETRY", "缩放/拖动期间旧 Metal generation 必须透明且不得提交 pending geometry；稳定后完整恢复 HDR。", "功能正确性", "system", "端到端验证拖动残影与旧 DNG 局部帧修复。", "真实 Cocoa Window 与确定性交互 driver 可用。", "JPEG 的 zoom×4 与 12 步双轴 scrollbar 移动。", "采集 geometry-staged、reset、render、finished telemetry。", "pending 期间 fallback=true/opacity=0/无 render；generation reset≥1；稳定后 prepared active、opacity=1、HDR target>1。", "driver 自动停止且窗口关闭。", "tests/hdr_quality_system.py"),
