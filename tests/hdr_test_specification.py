...
CASES = [
    ...
    case("ST-HDR-STAGED-FIRST-FRAME", "最终布局和首个 drawable 呈现前不得移除 SDR 占位或显示 Metal layer。", "功能正确性", "static", "防止 JPEG 黑块与旧几何首帧。", "view/renderer 源码可读。", "布局 gate、SDR proxy、layer opacity 和 presented handler。", "检查 fit 后才 arm、proxy 常驻到 presented、Metal 从 opacity 0 切到 1。", "未呈现阶段始终有正确占位，且不会暴露空 drawable。", "不创建窗口。", "tests/hdr_quality_static.py"),
    case("ST-HDR-OFFSCREEN-PREPARATION", "HDR 昂贵首求值必须在可见渐亮开始前离屏完成并缓存 float endpoint。", "时间行为", "static", "防止 RAW 首求值吞掉过渡窗口。", "renderer 源码可读。", "SDR/HDR 两张 RGBA16Float Metal endpoint。", "检查首帧后调度离屏 render、双纹理缓存以及 prepared endpoint 合成。", "过渡帧不再重复求值 48MP RAW 图。", "缓存随图像句柄销毁。", "tests/hdr_quality_static.py"),
    ...
    case("SYS-HDR-NO-PREMATURE-BLACK-FRAME", "首个 Metal 帧实际呈现前必须由 SDR proxy 覆盖且 layer opacity=0。", "功能正确性", "system", "端到端验证 JPEG 黑块修复。", "真实 Cocoa Window 与 telemetry 可用。", "JPEG/DNG 共 6 次真实启动记录。", "检查所有 presented=false 记录的 fallback/opacity，并检查 opacity>0 的呈现状态与几何。", "空 drawable 永不裸露；Metal 只在完整帧呈现后显示。", "关闭测试窗口。", "tests/hdr_quality_system.py"),
    case("SYS-HDR-FINAL-LAYOUT-BEFORE-METAL", "任何 Metal 提交都必须使用 fit 完成后的稳定 zoom 和最终 drawable geometry。", "功能正确性", "system", "端到端验证 DNG 左上角局部闪现修复。", "真实 Cocoa Window 与 DNG 可用。", "三次 DNG 启动的全部 telemetry。", "验证 layout_ready、drawable match，并比较每次运行从首提交开始的 zoom 集合。", "所有提交 layout-ready；每次 DNG zoom 从首帧起唯一稳定。", "关闭测试窗口。", "tests/hdr_quality_system.py"),
