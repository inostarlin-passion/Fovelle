...
CASES = [
    ...
    case("ST-HDR-OFFSCREEN-PREPARATION", "HDR 昂贵首求值与代表性 4× zoom ROI 必须在可见渐亮前由 Core Image 管理的可缓存 float intermediate 完成。", "时间行为", "static", "既防止 RAW 首求值/首次 zoom ROI 编码占用交互窗口，也防止自有纹理反向导入产生未解析 GPU tile。", "renderer 源码可读。", "SDR/HDR Core Image managed intermediates、动态 ramp、4× ROI 与串行预热 command buffer。", "检查 cacheIntermediates=YES、imageByInsertingIntermediate:YES、ramp/interactionWarmCorners 预热、prepared 合成，并确认 renderer 不调用 imageWithMTLTexture。", "过渡与首次放大复用已编译的 Core Image 浮点 workset；应用自有 render-target 从不作为后续 CI 输入。", "缓存只随图像替换销毁，交互几何变换继续复用。", "tests/hdr_quality_static.py"),
    ...
    case("ST-HDR-DISPLAYLINK-LATEST-ONLY", "Metal drawable 必须由 CAMetalDisplayLink 提供，CI 编码必须离开 AppKit 线程，且最多一帧在途、待处理状态永远覆盖为最新几何。", "时间行为", "static", "消除 nextDrawable/CI 编码的主线程阻塞与 NEF 缩放旧帧排队残影。", "macOS 14 SDK 与 renderer 源码可读。", "连续 zoom/pan 请求、专用串行 encode queue 与 GPU 帧。", "检查无 nextDrawable、DisplayLink drawable、dispatch serial encode、frameInFlight gate、generation 覆盖及 command-buffer ordered present。", "旧 generation 不会排队后再显示；AppKit 线程既不等待 drawable，也不执行 CI texture 编码。", "不启动显示循环。", "tests/hdr_quality_static.py"),
    ...
    case("ST-HDR-NAV-NO-OFFSCREEN-SURFACE", "悬浮导航按钮不得用 QGraphicsOpacityEffect 矩形离屏表面；透明 backing 内只淡入自身像素。", "功能正确性", "static", "消除 HDR Metal sibling 上提前出现/延后消失的标准矩形。", "主窗口源码可读。", "两个导航按钮与 opacity 动画。", "检查透明/no-system-background、paintOpacity 内绘、动画目标属性与无 graphics effect。", "圆角 artwork 与透明区同一时间过渡。", "不创建按钮。", "tests/hdr_quality_static.py"),
    ...
    case("SYS-HDR-SMOOTH-ACTIVATION", "打开 HDR 图后的内容强度必须按已提交帧平滑单调到 1，即使中间请求被合并也不得突跳。", "功能正确性", "system", "验证类似 Quick Look 的渐亮激活与冷帧节拍。", "DisplayLink、定时器和 telemetry 可用。", "每次启动的实际 submitted transition_progress。", "比较首帧、末帧与所有相邻采样。", "首帧≤0.1、末帧≥0.999、全程单调、任一相邻步≤0.15。", "约 1.8s 轮询停止；内部 paced follow-up 已收敛。", "tests/hdr_quality_system.py"),
    ...
    case("SYS-HDR-DISPLAYLINK-LATEST-ONLY", "JPEG 与 NEF 交互必须由 CAMetalDisplayLink 驱动、在专用队列编码，且停止后提交代际收敛到最新请求、无在途旧帧。", "时间行为", "system", "端到端验证 drawable 调度、主线程隔离与最新帧覆盖策略。", "真实 Cocoa/Metal 窗口和交互 driver 可用。", "JPEG、NEF 的连续 zoom/pan telemetry。", "聚合 display-link/off-main flag、callback、frame gate 与 requested/submitted generation。", "所有记录 usesDisplayLink/encodesOffMain=true；两次交互 settled 时 requested=submitted 且 frameInFlight=false。", "关闭窗口并保存调度计数。", "tests/hdr_quality_system.py"),
    case("SYS-HDR-INTERACTION-RESPONSIVENESS", "放大后的 JPEG 与 NEF 拖动必须满足主线程平均/P99/最大时延和吞吐量阈值。", "时间行为", "system", "量化拖动卡顿和悬浮控件事件循环响应风险。", "真实窗口、确定性 12ms pan driver 可用。", "JPEG/NEF 各一次 zoom×4 与 12 步双轴 pan。", "读取 zoom 同步耗时和相邻 pan 回调间隔，计算 average/P99/max/steps-per-second。", "两格式 zoom≤30ms、average≤25ms、P99≤45ms、max≤60ms、吞吐≥40 steps/s，步骤 0…11 无丢失。", "driver 停止并保存原始时间戳。", "tests/hdr_quality_system.py"),
    ...
    case("SYS-HDR-NAV-TRANSPARENT-SURFACE", "HDR 视口上的悬浮导航按钮在半透明过渡时不得暴露标准矩形 backing。", "功能正确性", "system", "以真实屏幕像素验证只有圆角 artwork 参与淡入淡出。", "JPEG HDR 窗口、测试 probe 与 screencapture 可用。", "真实右侧按钮 paintOpacity=0.5 的两帧和隐藏参考帧。", "读取按钮全局矩形，在四个 5pt 透明角与中心分别计算 RGB MAE。", "无 QGraphicsEffect；四角最大差≤3，绘制中心差≥1，并观察到 hidden 事件。", "隐藏按钮，保留三帧与逐 patch 指标。", "tests/hdr_quality_system.py"),
