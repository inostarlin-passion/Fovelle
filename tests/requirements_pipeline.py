...
CASES = [
    ...
    case("ST-SETTINGS-TAB-LAYOUT", "Settings 使用 AppKit preference toolbar 呈现 Preview 风格导航。", "验证原生 toolbar、隐藏 Qt 页模型和 stackedWidget 的单一选择链。", "Options 与 Cocoa bridge 可读。", "NSToolbar、NSWindowToolbarStylePreference、categoryTabs、stackedWidget。", "检查 QTabBar 仅作为隐藏页模型，原生 toolbar 使用图标+文字并同步 pane/title。", "系统 toolbar 居中显示六个可选 pane，当前 pane 高亮且窗口标题同步。", "不依赖自绘 tab 外观。", "static", "tests/requirements_pipeline.py::static_contracts"),
    ...
    case("ST-DIALOG-NATIVE-THEME", "Settings/About/图片信息页的 AppKit appearance 与 Qt palette 同步。", "验证应用级 Cocoa color scheme 与共享弹窗适配器。", "Application、Dialog 与 Cocoa bridge 源码可读。", "Light、Dark、System 设置。", "检查 setApplicationTheme/QStyleHints、窗口继承、applyTheme 和 palette 单元断言。", "Light/Dark 同时改变标题栏、内容背景、文字和控件状态；System 继承系统。", "只有 NSApp 一份主题状态。", "static", "tests/requirements_pipeline.py::static_contracts"),
    case("ST-URL-NATIVE", "File→Open URL 在 Light/Dark 下使用不同的系统 palette。", "验证 URL 输入、下载进度及错误 UI 的完整主题链。", "MainWindow、NativeDialogs 与 palette 测试可读。", "QInputDialog、QProgressDialog、错误消息及 Window/Base/Text roles。", "检查 pickUrl/openUrl 的共享主题入口，并运行真实 pickUrl palette 断言。", "输入、进度和错误弹窗继承 NSApp scheme；输入框和文字随主题翻转。", "网络请求取消时释放对话框。", "static", "tests/requirements_pipeline.py::static_contracts"),
    ...
    case("UT-NATIVE-DIALOGS", "Light/Dark 下 Settings/About/Info/MessageBox/Open URL 的 appearance 与 palette 正确。", "验证应用级主题和共享弹窗适配器。", "Cocoa test binary 可运行。", "Light/Dark QWidget dialogs 与真实 pickUrl。", "运行 testNativeDialogsFollowSelectedTheme 和 testOpenUrlDialogFollowsSelectedTheme。", "所有 Dialog 均得到 Aqua/DarkAqua，且 Window/Base/Text roles 明暗翻转。", "关闭并释放弹窗，恢复 Theme。", "unit", "tests/tst_qviewtests.cpp::WindowBehaviorTests::testNativeDialogsFollowSelectedTheme;testOpenUrlDialogFollowsSelectedTheme"),
...
UNIT_METHODS = {
    ...
    "UT-NATIVE-DIALOGS": ("WindowBehaviorTests::testNativeDialogsFollowSelectedTheme", "WindowBehaviorTests::testOpenUrlDialogFollowsSelectedTheme"),
...
def static_contracts(repo: Path) -> dict[str, dict]:
    ...
    native_dialogs = read(repo, "src/nativedialogs.cpp")
    ...
    checks = {
        ...
        "ST-SETTINGS-TAB-LAYOUT": {"passed": ui_parse and widgets.get("categoryTabs") == "QTabBar" and all(marker in cocoa + options + test_source for marker in ("NSWindowToolbarStylePreference", "NSToolbarDisplayModeIconAndLabel", "configureSettingsToolbar", "ui->categoryTabs->hide()", "hasNativeSettingsToolbar")), "actual": {"xml_parse": ui_parse, "category_model": widgets.get("categoryTabs"), "native_preference_toolbar": "NSWindowToolbarStylePreference" in cocoa, "hidden_qt_model": "ui->categoryTabs->hide()" in options}},
        ...
        "ST-DIALOG-NATIVE-THEME": {"passed": all(popup_coverage.values()) and all(marker in cocoa + cocoa_header + application + native_dialogs + test_source for marker in ("setApplicationTheme", "QStyleHints", "setColorScheme", "setAppearance:nil", "QPalette::Window", "QPalette::WindowText")), "actual": {"popup_coverage": popup_coverage, "application_scheme": "setApplicationTheme" in application, "palette_assertions": all(marker in test_source for marker in ("QPalette::Window", "QPalette::WindowText"))}},
        "ST-URL-NATIVE": {"passed": all(marker in read(repo, "src/mainwindow.cpp") + cocoa + test_source for marker in ("pickUrl", "QInputDialog", "QProgressDialog", "NativeDialogs::applyTheme", "testOpenUrlDialogFollowsSelectedTheme", "QPalette::Base")), "actual": {"input": "QInputDialog" in read(repo, "src/mainwindow.cpp"), "progress": "QProgressDialog" in read(repo, "src/mainwindow.cpp"), "palette_regression_test": "testOpenUrlDialogFollowsSelectedTheme" in test_source}},
        ...
        "Q-TESTABLE": {"passed": all(marker in test_source for marker in ("ScopedEnvironmentValue", "testNativeDialogsFollowSelectedTheme", "testOpenUrlDialogFollowsSelectedTheme", "testSystemThemeResolvesFromControlledAppearance")) and "FOVELLE_DIAGNOSTIC_LOG" in read(repo, "tests/eps_quality_system.py"), "actual": {"controlled_theme": "ScopedEnvironmentValue" in test_source, "dialog_palette_regression": "testOpenUrlDialogFollowsSelectedTheme" in test_source, "system_telemetry": "FOVELLE_DIAGNOSTIC_LOG" in read(repo, "tests/eps_quality_system.py")}},
    }
    return checks
