...
FUNCTIONAL_CASES = (
    ...
    case("TC-APP-VERSION", "应用版本与构建版本一致", "验证发布版本契约。", "CMake/qmake 版本定义可读；应用已初始化。", "读取 VERSION/VERSION_STRING 和运行时版本。", "执行 FeatureTests::testApplicationVersionIsCurrent。", "所有版本来源为 0.1.1 且一致。", "不修改版本配置。", "unit", "tests/tst_qviewtests.cpp::FeatureTests::testApplicationVersionIsCurrent", ("unit.json", "static.json")),
...
QUALITY_CASES = (
    case("AC-LEAN-01", "需求范围内的行为、CI 修复和审计产物均有明确边界", "检查实现没有遗漏用户要求，也没有把无关功能纳入变更。", "仓库工作树可读；当前任务范围为源代码、测试、CI 与 reports。", "git diff --check、静态检查结果、文件清单。", "运行 quality_static.py 与 quality_integration.py，核对变更路径和检查 ID。", "所有任务范围检查通过；未发现越界文件；README 既有改动不被覆盖。", "保留用户已有 README 改动；reports 证据可追溯。", "static/integration", "tests/quality_static.py::ST-03/ST-25; tests/quality_integration.py", ("static.json", "integration.json", "manifest.json")),
    case("AC-FUNC-01", "规定输入产生规定输出、副作用并保持不变量", "汇总 43 个功能原子验收点的确定性执行。", "可执行测试二进制已构建；Cocoa 环境变量已设置。", "6 个 Qt Test suite、43 个 test case。", "运行 quality_unit_runner.py，按 suite 和 case 解析 Qt Test 文本输出。", "43/43 case passed；6/6 suite passed；failed/skipped/blacklisted 均为 0。", "测试进程返回码为 0；输出被保存到 unit.json。", "unit", "tests/quality_unit_runner.py; tests/tst_qviewtests.cpp", ("unit.json", "static.json")),
    case("AC-TIME-01", "目标工作负载的平均、P99、最大响应时间和吞吐量满足测试合同", "测量 native gesture 及应用启动/加载工作负载的时间行为。", "macOS Cocoa app 已构建；计时器、ps/lsof 可用；目标阈值来自 quality_system_probe.py。", "240 gesture events；4 类 native/oriented image case，每类 3 次启动。", "运行 quality_feature_system.py 和 quality_system_probe.py，解析 average/p99/max/throughput。", "gesture 计时字段齐全且 count=240；system probe 的 startup average/p99/max、throughput、资源上限全部 pass。", "所有被测进程按协议退出；原始观测和阈值保存在 JSON。", "system/performance", "tests/quality_feature_system.py; tests/quality_system_probe.py", ("system_feature.json", "system_probe.json")),
    case("AC-TIME-02", "异常慢测试不会无限占用 CI 资源", "验证 Qt function、CTest、workflow job 三层时间边界。", "CMakeLists 和 workflow 文件可读。", "QTEST_FUNCTION_TIMEOUT=30000、CTest TIMEOUT/--timeout=90、Actions timeout-minutes。", "运行 static check ST-25，并检查配置中的超时常量。", "所有时间边界存在且值符合 30s/90s/10min 等项目合同。", "未来挂死时产生可诊断失败而非无限等待。", "static/CI", "tests/quality_static.py::ST-25-CI-TEST-BOUNDS", ("static.json", "ci-failure.json", "manifest.json")),
    case("AC-TEST-01", "测试用例可系统化生成并覆盖实现中的每个功能 case", "验证规格和可执行测试清单一一对应。", "quality_unit_runner.EXPECTED_CASES 和规格脚本均可导入。", "43 个 case ID、suite、test method 映射。", "运行 quality_specification.py 的映射校验。", "每个 EXPECTED_CASES ID 恰好有一个完整规格条目，方法名和证据引用非空。", "输出 test-specification.json/markdown 可供审计。", "static/specification", "tests/quality_specification.py", ("test-specification.json", "static.json", "manifest.json")),
    ...
    case("AC-TEST-03", "运行时状态可非侵入式观测并可复核外部输出", "验证诊断 telemetry、资源采样和原始 stdout/stderr 均被保留。", "app 可启动；诊断环境变量和系统采样工具可用。", "FOVELLE_DIAGNOSTIC_LOG=1、FOVELLE_VIEW 行、ps/lsof/netstat/iostat。", "运行 layout/system probes 并检查 JSON 中 raw_output、observations、metrics、thresholds。", "输出包含 geometry、资源、网络/磁盘主机观测及其 limitations；无需修改业务状态。", "子进程收到 SIGTERM/退出，原始证据文件可通过 hash 复核。", "system/audit", "tests/quality_layout_system.py; tests/quality_system_probe.py", ("system_layout.json", "system_probe.json", "manifest.json")),
    case("AC-TEST-04", "几何回归测试不依赖 Retina 专属显示比例", "验证同一测试在 Retina 与非 Retina Cocoa 条件下使用相同业务不变量。", "Cocoa 测试二进制已构建；可控制 QT_SCALE_FACTOR。", "默认显示比例和 QT_SCALE_FACTOR=1 两个运行条件。", "分别运行 quality_unit_runner.py 默认模式和 --scale-factor 1 模式，比较 GraphicsViewTests 及总套件结果。", "两种条件均为 67 passed、0 failed、0 skipped；断言只要求图像非空且不小于原始 1200px，不要求严格大于 1200px。", "两个测试进程均返回 0；证据保留各自 environment。", "unit/cross-platform", "tests/quality_unit_runner.py; tests/tst_qviewtests.cpp::GraphicsViewTests::testImageIsCenteredAfterOpeningWithScrollBars", ("unit.json", "unit_scale1.json", "static.json", "manifest.json")),
    case("AC-CI-01", "根因修复覆盖 re-entrant scene rect 路径而非只延长等待", "验证修复直接针对失败调用链。", "QVGraphicsView 源代码可读；旧 CI failure evidence 已固化。", "resizeEvent/updateSceneRect/sceneRect 的控制流和 Qt 文档约束。", "运行静态 ST-25，执行所有 Qt tests，并阅读 ci-failure.json 的 root_cause_inference。", "存在 isUpdatingSceneRect guard、QScopedValueRollback、redundant setSceneRect skip；43 个 case 和 CTest 通过。", "未修改业务输入输出契约；失败分析中的不确定性仍显式保留。", "static/unit/integration", "src/qvgraphicsview.cpp; src/qvgraphicsview.h; tests/quality_static.py", ("ci-failure.json", "static.json", "unit.json", "integration.json")),
...
def markdown(record: dict) -> str:
    lines = [
        ...
        "- 机器可读源：[evidence/test-specification.json](evidence/test-specification.json)",
        "",
        "本规格将用户提出的四项代码质量要求拆解为质量属性验收点，并将实现功能拆解为 43 个可执行 Qt Test case。每个条目均明确测试目的、前置条件、输入数据、操作步骤、预期结果和后置条件；`implementation` 指向实际测试代码或质量门禁。",
        "",
        "## 原子验收测试矩阵",
        "",
    ]
