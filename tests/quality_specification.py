...
FUNCTIONAL_CASES = (
    ...
    case("TC-SETTINGS-GROUPS-DEFAULTS", "General 选项按八组顺序排列且默认值正确", "验证 General 页的语义分组以及 Appearance/Smooth scaling 默认值。", "Options dialog 与 SettingsManager 已初始化。", "八组 object metadata、Dark 和 Bilinear 默认值。", "执行 FeatureTests::testSettingsGeneralGroupsAndDefaults。", "八组成员和顺序与需求完全一致；默认 Appearance=Dark、Smooth scaling=Bilinear。", "对话框销毁；持久化设置恢复。", "unit", "tests/tst_qviewtests.cpp::FeatureTests::testSettingsGeneralGroupsAndDefaults", ("unit.json", "static.json")),
    case("TC-PRELOAD-FORCED-ADJACENT-DISABLED", "preloadingmode=0 不能关闭相邻预加载", "验证旧 Disabled 持久化值被固定 Adjacent 策略覆盖。", "Cocoa QtTest 环境可用；临时四图序列可创建；QVImageLoader 可观察。", "preloadingmode=0；当前图 priority=0；相邻图 priority=1；距离 2 图。", "执行 FeatureTests::testPreloadingIgnoresDisabledUserSetting，打开当前图并收集 loadStarted。", "当前图和直接相邻图均被请求；没有距离 2 图或 priority>1 的请求。", "窗口关闭；原始设置恢复；线程池任务收敛。", "unit", "tests/tst_qviewtests.cpp::FeatureTests::testPreloadingIgnoresDisabledUserSetting", ("unit.json", "static.json")),
    case("TC-PRELOAD-FORCED-ADJACENT-EXTENDED", "preloadingmode=2 不能扩展到非相邻预加载", "验证旧 Extended 持久化值被固定 Adjacent 策略覆盖。", "Cocoa QtTest 环境可用；临时四图序列可创建；QVImageLoader 可观察。", "preloadingmode=2；当前图两侧直接邻居；一张距离 2 图。", "执行 FeatureTests::testPreloadingIgnoresExtendedUserSetting，打开第二张图并收集 loadStarted。", "两侧直接邻居均以 priority=1 请求；距离 2 图不存在；没有 priority>1。", "窗口关闭；原始设置恢复；线程池任务收敛。", "unit", "tests/tst_qviewtests.cpp::FeatureTests::testPreloadingIgnoresExtendedUserSetting", ("unit.json", "static.json")),
    case("TC-SETTINGS-SPACING", "General 与 Mouse 的组间距大于组内选项间距", "验证组间距由原生样式度量推导，并在两个页面保持一致。", "Cocoa QPA、QMacStyle 和 Settings 对话框可用。", "settingsGroupSpacing、settingsIntraGroupMaxSpacing、每个 QFormLayout 的 verticalSpacing 及直接布局项几何。", "执行 WindowBehaviorTests::testSettingsSpacingUsesNativeStyle，切换 Mouse 的 Click/Drag 模式并重复扫描。", "两个页面的组间距相同且严格大于组内最大间距；表单使用 verticalSpacing=-1；页面末尾仅有 bottom stretch。", "对话框关闭；Mouse 模式和设置值恢复。", "integration", "tests/tst_qviewtests.cpp::WindowBehaviorTests::testSettingsSpacingUsesNativeStyle", ("integration.json", "static.json")),
    ...
    case("TC-APP-VERSION", "应用版本与构建版本一致", "验证发布版本契约。", "CMake/qmake 版本定义可读；应用已初始化。", "读取 VERSION/VERSION_STRING 和运行时版本。", "执行 FeatureTests::testApplicationVersionIsCurrent。", f"所有版本来源为 {PROJECT_VERSION} 且一致。", "不修改版本配置。", "unit", "tests/tst_qviewtests.cpp::FeatureTests::testApplicationVersionIsCurrent", ("unit.json", "static.json")),
    ...
    case("TC-SETTINGS-NATIVE-TOOLBAR", "Settings 使用 native toolbar tab contract 且切换立即持久化", "验证 macOS 设置工具栏与 Qt 页面状态保持一致。", "Cocoa window handle 可用；Settings 对话框和 toolbar bridge 已初始化。", "切换 General/Shortcuts/Mouse tab 并读取 optionstab。", "执行 WindowBehaviorTests::testSettingsDialogUsesNativeTabContractAndImmediatePersistence。", "toolbar tab、Qt stacked page 和持久化 index 一致；切换后立即生效。", "关闭对话框并恢复 optionstab/optionstabversion。", "unit", "tests/tst_qviewtests.cpp::WindowBehaviorTests::testSettingsDialogUsesNativeTabContractAndImmediatePersistence", ("unit.json", "integration.json")),
    case("TC-OPEN-URL-THEME", "打开 URL 对话框遵循选定主题", "验证主题切换后的原生 URL 对话框外观契约。", "Cocoa application 与 URL dialog 可构造；Settings theme 可控。", "分别选择 Light 和 Dark 并读取 dialog appearance/palette。", "执行 WindowBehaviorTests::testOpenUrlDialogFollowsSelectedTheme。", "URL dialog 的 native appearance 与选定主题一致。", "关闭 dialog 并恢复原主题。", "unit", "tests/tst_qviewtests.cpp::WindowBehaviorTests::testOpenUrlDialogFollowsSelectedTheme", ("unit.json", "system_feature.json")),
    ...
    case("TC-SETTINGS-ALL-LANGUAGES-TABS", "所有语言下所有 Settings Tab 的可见选项均完整显示", "验证长复选框文本和全部控件不会被 General 页宽度或其他 Tab 的布局约束裁切。", "Cocoa Qt Test 应用、五个语言目录和 Settings 对话框可用。", "en、es、ja、zh_Hans、zh_Hant；General/Shortcuts/Mouse；Mouse 两种模式。", "执行 WindowBehaviorTests::testSettingsEveryTabFitsEveryLanguage。", "每种语言和每个 Tab 均无水平滚动；所有可见控件宽度和高度不小于最终自然尺寸且完全落在 viewport 内。", "对话框、翻译器和设置值恢复。", "system", "tests/tst_qviewtests.cpp::WindowBehaviorTests::testSettingsEveryTabFitsEveryLanguage", ("system_feature.json", "static.json")),
    case("TC-SETTINGS-FORM-ALIGNMENT", "General 和 Mouse 每个设置表单的标签右对齐、值左对齐", "验证参考图所示两列网格在所有支持语言和 Mouse 模式下保持稳定。", "Cocoa Qt Test 应用、五个语言目录和 Settings 对话框可用。", "每个 General/Mouse QFormLayout 的 label/value item alignment 与运行时几何。", "执行 WindowBehaviorTests::testSettingsFormsAlignLabelsAndValues。", "标签文本右对齐到共享列末端；值项从字段列左侧开始；无独立表单水平居中；Associate action 仍居中。", "对话框、翻译器和设置值恢复。", "integration", "tests/tst_qviewtests.cpp::WindowBehaviorTests::testSettingsFormsAlignLabelsAndValues", ("integration.json", "static.json")),
    case("TC-SETTINGS-ASSOCIATE-NATIVE-STYLE", "Associate all supported formats 保持直接、稳定的原生按钮结构", "验证关联操作按钮没有被 stretch 子布局或主题 QSS 改变结构和自然尺寸。", "Cocoa Qt Test 应用和 Settings 对话框可用；不执行真实文件关联操作。", "associateFormatsButton 的 QFormLayout role/alignment、stylesheet、flat/autoDefault/default、minimum width、sizeHint 和 geometry。", "执行 WindowBehaviorTests::testSettingsAssociateButtonUsesNativeStyle。", "按钮直接位于 settingsGroup8 的 SpanningRole，水平/垂直居中、stylesheet 为空、非 flat、autoDefault/default 均为 true，且没有人工最小宽度。", "对话框关闭；真实文件关联操作未触发。", "integration", "tests/tst_qviewtests.cpp::WindowBehaviorTests::testSettingsAssociateButtonUsesNativeStyle", ("integration.json", "static.json")),
...
QUALITY_CASES = (
    ...
    case("AC-FUNC-01", "规定输入产生规定输出、副作用并保持不变量", "汇总 55 个功能原子验收点的确定性执行。", "可执行测试二进制已构建；Cocoa 环境变量已设置。", "6 个 Qt Test suite、55 个功能 test case。", "运行 quality_unit_runner.py，按 suite 和 case 解析 Qt Test 文本输出。", "55/55 case passed；6/6 suite passed；failed/skipped/blacklisted 均为 0。", "测试进程返回码为 0；输出被保存到 unit.json。", "unit", "tests/quality_unit_runner.py; tests/tst_qviewtests.cpp", ("unit.json", "static.json")),
    ...
    case("AC-RAW-PIPELINE-01", "RAW 使用 Image I/O、Core Image RAW、ColorSync 和 Metal/CIContext 的系统管线", "验证 RAW 文件识别不依赖扩展名，并且解码、色彩管理、GPU 渲染和预览降级均有原生 API 契约。", "macOS SDK 提供 Image I/O、Core Image、ColorSync、Metal；native bridge 和 ImageLoader 源码可读；当前验收环境提供真实 sample1.nef。", "CGImageSource UTI、CIRAWFilter outputImage/previewImage、ColorSync sRGB profile、Metal-backed CIContext，以及 /Users/inostarlin/Downloads/sample1.nef。", "运行 quality_static.py 的 ST-06、quality_integration.py 的 I-03，并使用 quality_system_probe.py 加载 sample1.nef 检查诊断 contentRect。", "静态证据同时包含 CGImageSourceGetType、filterWithImageURL、previewImage、ColorSync profile、working/output color space、MTLCreateSystemDefaultDevice/contextWithMTLDevice；RAW 分支不读取文件扩展名；sample1.nef 产生非零 RGB 内容几何，无法解码时保留预览或返回可见错误。", "不改变用户设置；证据文件保留每个 marker、运行指标和样本路径；未覆盖的相机型号仍显式记录为不确定性。", "static/integration/system", "tests/quality_static.py::ST-06; src/qvcocoafunctions.mm; src/qvimageloader.cpp; tests/quality_system_probe.py", ("static.json", "integration.json", "system_raw_probe.json")),
    ...
    case("AC-TEST-01", "测试用例可系统化生成并覆盖实现中的每个功能 case", "验证规格和可执行测试清单一一对应。", "quality_unit_runner.EXPECTED_CASES 和规格脚本均可导入。", "55 个 case ID、suite、test method 映射。", "运行 quality_specification.py 的映射校验。", "每个 EXPECTED_CASES ID 恰好有一个完整规格条目，方法名和证据引用非空。", "输出 test-specification.json/markdown 可供审计。", "static/specification", "tests/quality_specification.py", ("test_specification.json", "static.json")),
    ...
    case("AC-TEST-04", "几何回归测试不依赖 Retina 专属显示比例", "验证同一测试在 Retina 与非 Retina Cocoa 条件下使用相同业务不变量。", "Cocoa 测试二进制已构建；可控制 QT_SCALE_FACTOR。", "默认显示比例和 QT_SCALE_FACTOR=1 两个运行条件。", "分别运行 quality_unit_runner.py 默认模式和 --scale-factor 1 模式，比较 GraphicsViewTests 及总套件结果。", "两种条件均为 135 passed、0 failed、0 skipped；断言只要求图像非空且不小于原始 1200px，不要求严格大于 1200px。", "两个测试进程均返回 0；证据保留各自 environment。", "unit/cross-platform", "tests/quality_unit_runner.py; tests/tst_qviewtests.cpp::GraphicsViewTests::testImageIsCenteredAfterOpeningWithScrollBars", ("unit.json", "unit_scale1.json", "static.json")),
    case("AC-CI-01", "根因修复覆盖 re-entrant scene rect 路径而非只延长等待", "验证修复直接针对失败调用链。", "QVGraphicsView 源代码可读；实现和当前集成证据可读取。", "resizeEvent/updateSceneRect/sceneRect 的控制流和 Qt 文档约束。", "运行静态 ST-25，执行所有 Qt tests，并核对 integration.json 中的控制流断言。", "存在 isUpdatingSceneRect guard、QScopedValueRollback、redundant setSceneRect skip；52 个 case 和 CTest 通过。", "未修改业务输入输出契约；实现证据和测试原始输出均保留。", "static/unit/integration", "src/qvgraphicsview.cpp; src/qvgraphicsview.h; tests/quality_static.py", ("static.json", "unit.json", "integration.json")),
...
def markdown(record: dict) -> str:
    lines = [
        ...
        "",
        "本规格将用户提出的代码质量要求拆解为质量属性验收点，并将实现功能拆解为 55 个可执行 Qt Test case。每个条目均明确测试目的、前置条件、输入数据、操作步骤、预期结果和后置条件；`implementation` 指向实际测试代码或质量门禁。",
        "",
        "## 原子验收测试矩阵",
        "",
    ]
