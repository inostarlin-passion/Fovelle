...
QUALITY_CASES = (
    ...
    case("AC-FUNC-01", "规定输入产生规定输出、副作用并保持不变量", "汇总 51 个功能原子验收点的确定性执行。", "可执行测试二进制已构建；Cocoa 环境变量已设置。", "6 个 Qt Test suite、51 个功能 test case。", "运行 quality_unit_runner.py，按 suite 和 case 解析 Qt Test 文本输出。", "51/51 case passed；6/6 suite passed；failed/skipped/blacklisted 均为 0。", "测试进程返回码为 0；输出被保存到 unit.json。", "unit", "tests/quality_unit_runner.py; tests/tst_qviewtests.cpp", ("unit.json", "static.json")),
    ...
    case("AC-TEST-01", "测试用例可系统化生成并覆盖实现中的每个功能 case", "验证规格和可执行测试清单一一对应。", "quality_unit_runner.EXPECTED_CASES 和规格脚本均可导入。", "51 个 case ID、suite、test method 映射。", "运行 quality_specification.py 的映射校验。", "每个 EXPECTED_CASES ID 恰好有一个完整规格条目，方法名和证据引用非空。", "输出 test-specification.json/markdown 可供审计。", "static/specification", "tests/quality_specification.py", ("test_specification.json", "static.json")),
    ...
    case("AC-CI-01", "根因修复覆盖 re-entrant scene rect 路径而非只延长等待", "验证修复直接针对失败调用链。", "QVGraphicsView 源代码可读；实现和当前集成证据可读取。", "resizeEvent/updateSceneRect/sceneRect 的控制流和 Qt 文档约束。", "运行静态 ST-25，执行所有 Qt tests，并核对 integration.json 中的控制流断言。", "存在 isUpdatingSceneRect guard、QScopedValueRollback、redundant setSceneRect skip；51 个 case 和 CTest 通过。", "未修改业务输入输出契约；实现证据和测试原始输出均保留。", "static/unit/integration", "src/qvgraphicsview.cpp; src/qvgraphicsview.h; tests/quality_static.py", ("static.json", "unit.json", "integration.json")),
...
def markdown(record: dict) -> str:
    lines = [
        ...
        "",
        "本规格将用户提出的四项代码质量要求拆解为质量属性验收点，并将实现功能拆解为 51 个可执行 Qt Test case。每个条目均明确测试目的、前置条件、输入数据、操作步骤、预期结果和后置条件；`implementation` 指向实际测试代码或质量门禁。",
        "",
        "## 原子验收测试矩阵",
        "",
    ]
