...
FUNCTIONAL_CASES = (
    ...
    case("TC-SETTINGS-GROUPS-DEFAULTS", "General 选项按八组顺序排列且默认值正确", "验证 General 页的语义分组、组间距和 Appearance/Smooth scaling 默认值。", "Options dialog 与 SettingsManager 已初始化。", "八组 object metadata、6 px 行距、18 px 组间距、Dark 和 Bilinear 默认值。", "执行 FeatureTests::testSettingsGeneralGroupsAndDefaults。", "八组成员和顺序与需求完全一致；组间距大于组内行距；默认 Appearance=Dark、Smooth scaling=Bilinear。", "对话框销毁；持久化设置恢复。", "unit", "tests/tst_qviewtests.cpp::FeatureTests::testSettingsGeneralGroupsAndDefaults", ("unit.json", "static.json")),
    ...
    case("TC-APP-VERSION", "应用版本与构建版本一致", "验证发布版本契约。", "CMake/qmake 版本定义可读；应用已初始化。", "读取 VERSION/VERSION_STRING 和运行时版本。", "执行 FeatureTests::testApplicationVersionIsCurrent。", "所有版本来源为 1.0.1 且一致。", "不修改版本配置。", "unit", "tests/tst_qviewtests.cpp::FeatureTests::testApplicationVersionIsCurrent", ("unit.json", "static.json")),
    ...
    case("TC-SETTINGS-ASSOCIATE-NATIVE-STYLE", "Associate all supported formats 保持直接、稳定的主操作按钮结构", "验证关联操作按钮没有被 stretch 子布局或 QDialog auto-default 改变结构和自然尺寸。", "Cocoa Qt Test 应用和 Settings 对话框可用；不执行真实文件关联操作。", "associateFormatsButton 的 QFormLayout role/alignment、flat/autoDefault/default、stylesheet、sizeHint 和 geometry。", "执行 WindowBehaviorTests::testSettingsAssociateButtonUsesNativeStyle。", "按钮直接位于 settingsGroup8 的 SpanningRole，水平/垂直居中、非 flat、非 auto-default，且满足原生最小尺寸。", "对话框关闭；真实文件关联操作未触发。", "integration", "tests/tst_qviewtests.cpp::WindowBehaviorTests::testSettingsAssociateButtonUsesNativeStyle", ("integration.json", "static.json")),
    case("TC-SETTINGS-ASSOCIATE-THEME", "Associate all supported formats 在浅色和深色模式下均为蓝色填充、白字圆角样式", "验证此前的蓝色主操作语义不会因布局重构或主题切换退化为普通黑/灰色按钮。", "Cocoa Qt Test 应用、Light/Dark 主题、有效 QPalette Accent 和可抓取的按钮渲染均可用；不执行真实文件关联操作。", "Qv::Theme::Light、Qv::Theme::Dark；QPalette::Accent/Highlight；按钮 stylesheet、动态属性和 QImage 渲染像素。", "分别设置 Light 与 Dark，显示 Settings，等待有效 palette，检查 accent-filled、背景色、白/高亮文字、无边框、圆角属性，并扫描按钮截图中的 Accent 像素。", "两种主题都保持 Accent 派生的蓝色填充、白色或高亮文字、6 px 圆角和无边框；按钮渲染中存在足量 Accent 背景像素，不再显示普通灰色按钮。", "关闭每个对话框并恢复临时主题设置；不触发真实文件关联操作。", "integration", "tests/tst_qviewtests.cpp::WindowBehaviorTests::testSettingsAssociateButtonFollowsThemeAccent", ("integration.json", "static.json")),
...
QUALITY_CASES = (
    ...
    case("AC-FUNC-01", "规定输入产生规定输出、副作用并保持不变量", "汇总 53 个功能原子验收点的确定性执行。", "可执行测试二进制已构建；Cocoa 环境变量已设置。", "6 个 Qt Test suite、53 个功能 test case。", "运行 quality_unit_runner.py，按 suite 和 case 解析 Qt Test 文本输出。", "53/53 case passed；6/6 suite passed；failed/skipped/blacklisted 均为 0。", "测试进程返回码为 0；输出被保存到 unit.json。", "unit", "tests/quality_unit_runner.py; tests/tst_qviewtests.cpp", ("unit.json", "static.json")),
    ...
    case("AC-TEST-01", "测试用例可系统化生成并覆盖实现中的每个功能 case", "验证规格和可执行测试清单一一对应。", "quality_unit_runner.EXPECTED_CASES 和规格脚本均可导入。", "53 个 case ID、suite、test method 映射。", "运行 quality_specification.py 的映射校验。", "每个 EXPECTED_CASES ID 恰好有一个完整规格条目，方法名和证据引用非空。", "输出 test-specification.json/markdown 可供审计。", "static/specification", "tests/quality_specification.py", ("test_specification.json", "static.json")),
...
def markdown(record: dict) -> str:
    lines = [
        ...
        "",
        "本规格将用户提出的四项代码质量要求拆解为质量属性验收点，并将实现功能拆解为 53 个可执行 Qt Test case。每个条目均明确测试目的、前置条件、输入数据、操作步骤、预期结果和后置条件；`implementation` 指向实际测试代码或质量门禁。",
        "",
        "## 原子验收测试矩阵",
        "",
    ]
