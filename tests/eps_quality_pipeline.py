...
def build_specification(repo: Path, generated_at: str) -> dict:
    cases = [
        ...
        case(
            ...
            "EPS 场景项必须以当前 device transform 对 exposedRect 加有界拖动边界做终端密度绘制，且全部缩放入口统一限制为 3200%。",
            "功能正确性/时间行为",
            "static",
            ...
            ["检查可见区加 128 device-pixel pan border 的 PDF tile 计算。", "检查 75% 后台交互 tile 与 50ms 精确 idle refinement。", "检查中央 zoom clamp 和 3200% UI 上限。"],
            ["交互帧可变换最近 tile 并由后台追赶，50ms idle 后恢复精确矢量终端采样；wheel、pinch、键盘、会话和自定义缩放均不超过 32.0。"],
            ["静态检查不分配图像。"],
            "tests/eps_quality_static.py",
            "reports/evidence/eps_static.json",
        ),
...
def build_quality_report(
    ...
    return {
        ...
        "scope": "EPS/EPSF/EPSI authoritative vector-document rendering, 3200% zoom bound, format registration, static-document retention, dependency documentation, tests, and audit artifacts",
