...
def build_specification(repo: Path, generated_at: str) -> dict:
    cases = [
        ...
        case(
            "ST-EPS-TESTABILITY",
            "测试必须提供无内嵌预览的确定性 vector EPS、外部样例像素判据、native render、异步 loader、placement preview/转换缓存、延迟 movie probe、缺依赖错误、设置迁移和损坏输入用例。",
            "可测试性",
            "static",
            "验证每项实现边界都有可执行测试入口。",
            ["Qt test source 可读。"],
        ...
        case(
            "UT-EPS-PREVIEW",
            "含内嵌 placement preview 的 EPS 必须先返回标记为 provisional 的有界预览，再在同一 request id 上返回完整 PDF vector result；预览不能结束加载或触发 QMovie 探测。",
            "时间行为/功能正确性",
            "unit",
            "验证 EPS 首次可见延迟优化没有把预览误当成权威文档。",
            ["fovelle_tests 已编译。", "提供带 DOS TIFF preview 的 EPS 样例。"],
            ["外部 EPS 样例的唯一临时副本。"],
            ["调用 readPlacementPreview。", "请求 QVImageLoader 并记录首个与最终 imageReady。", "比较 provisional 标记、request id、PDF vectorImage 和 preview/final 墙钟时间。"],
            ["首个结果为 bounded preview 且 provisional=true；最终结果为有效 PDF；preview 时间至少比最终权威结果提前 50%。"],
            ["临时副本和 loader 资源释放。"],
            "tests/tst_qviewtests.cpp::ImageLoaderTests::testEPSPlacementPreviewIsProvisional",
            "reports/evidence/eps_unit.json",
        ),
        case(
            "UT-EPS-CACHE",
