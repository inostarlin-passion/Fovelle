...
def build_specification(repo: Path, generated_at: str) -> dict:
    cases = [
        ...
        case(
            ...
            "EPS 场景项必须异步生成当前 device transform 下带有界拖动边界的 exposedRect 图块，在拖动时使用小 seam guard、复用不透明 backing store，且全部缩放入口统一限制为 6400%。",
            "功能正确性/时间行为",
            "static",
            "验证高倍缩放没有退化为固定整图位图，并统一执行缩放上限。",
            ["graphics item、graphics view 与 zoom 对话框源码可读。"],
            ["NoCache、ItemUsesExtendedStyleOption、exposedRect、deviceTransform、QtConcurrent interaction refinement、16/128 device-pixel seam guard、2-entry/96 MiB tile limit、MaximumZoomLevel。"],
            ["检查可见区在拖动/idle 状态使用有界 seam guard 的 PDF tile 计算。", "检查交互与 idle 阶段均按当前 device density 异步生成 tile，并在复用时只对整数一一映射使用快速采样。", "检查矢量滚动的不透明局部重绘。", "检查中央 zoom clamp 和 6400% UI 上限。"],
        ...
        case(
            ...
            "QVImageLoader 必须复用 native Result 且不增加 suffix 分支；EPS 的第一份 Result 必须已经包含权威 PDF，QVImageCore 必须跳过矢量文档的 QMovie 探测。",
            "精益完整性/功能正确性",
            "static",
        ...
        case(
            "ST-EPS-TESTABILITY",
            "测试必须提供无内嵌预览的确定性 vector EPS、外部样例像素判据、native render、异步 loader、首帧/稳定帧一致性、转换缓存、延迟 movie probe、缺依赖错误、设置迁移和损坏输入用例。",
            "可测试性",
            "static",
            "验证每项实现边界都有可执行测试入口。",
            ["Qt test source 可读。"],
            ["EPS 测试方法声明、FOVELLE_EPS_SAMPLE、vector PostScript fixture、拖动与显示性能日志。"],
            ["检查测试方法和可控输入标记。"],
            ["静态检查能定位全部 EPS 测试入口。"],
            ["不执行测试进程。"],
            "tests/eps_quality_static.py",
        ...
        case(
            ...
            "功能正确性",
            "unit",
            "验证渲染来自 PostScript，而不是 120x40 的内嵌 placement preview。",
            ["Ghostscript 可执行；样例文件可读，或临时目录可写。"],
            ["用户提供的 DOS EPS；样例缺失时为包含黑底和两个白色 vector 方块的 8x4 EPS。"],
            ["调用 readImageWithImageIO。", "检查 PDF bytes、PDF-derived fallback、BoundingBox 逻辑尺寸。", "由持久 PDF document 请求 2048px 全页和上半页 tile。"],
        ...
        case(
            ...
            ["请求完成，vectorImage 为 PDF、fallback 最大边不超过 2048、logical size 独立保留且无 errorData。"],
            ["loader 和临时文件释放。"],
        ...
        case(
            "UT-EPS-CONSISTENCY",
            "含内嵌 placement preview 的 EPS 第一份 imageReady 结果必须已经是完整 PDF vector result；其 fallback 与同尺寸的 PDF 重绘误差必须小于 3，不能发布 provisional preview。",
            "功能正确性/时间行为",
            "unit",
            "验证 EPS 首帧和稳定帧来自同一权威源，消除打开瞬间的图像跳变。",
            ["fovelle_tests 已编译。", "提供带 DOS TIFF preview 的 EPS 样例或确定性 EPS。"],
            ["外部 EPS 样例的唯一临时副本。"],
            ["请求 QVImageLoader 并记录第一份 imageReady。", "检查 PDF vectorImage 和 fallback。", "以第一份 fallback 的尺寸从同一 PDF 重绘并比较采样通道误差。"],
            ["第一份结果为有效 PDF；没有 provisional preview；同尺寸 fallback 与 PDF 重绘的平均通道误差小于 3。"],
            ["临时副本、PDF document 和 loader 资源释放。"],
            "tests/tst_qviewtests.cpp::ImageLoaderTests::testEPSInitialFrameMatchesStableRender",
            "reports/evidence/eps_unit.json",
        ),
        case(
            "UT-EPS-CACHE",
            "同一 EPS identity 的第二次转换必须命中有界缓存，跳过 Ghostscript，并返回与 miss 相同的完整 PDF bytes；hit 时间不超过 miss 的 50%。",
            "时间行为/功能正确性",
            "unit",
            "验证 EPS 显示/转换延迟的可重复缓存路径和身份绑定。",
            ["fovelle_tests 已编译。", "Ghostscript 可执行。"],
            ["唯一临时 EPS 副本，先制造 miss 再请求同一 identity。"],
            ["调用 native bridge 两次。", "记录 miss/hit 墙钟时间。", "比较 PDF bytes、逻辑尺寸和 PDF vector document 有效性。"],
            ["第二次调用命中缓存；PDF bytes 不变；hit 时间小于 miss 的一半。"],
            ["临时副本和 native 资源释放。"],
            "tests/tst_qviewtests.cpp::ImageLoaderTests::testEPSRenderCacheCutsConversionLatency",
            "reports/evidence/eps_unit.json",
        ),
        case(
            "UT-EPS-STATIC",
            "QVImageCore 处理 EPS 后等待 1100ms，QMovie 必须保持 NotRunning，权威 PDF vectorImage 不得被异步动画探测替换。",
            "功能正确性/可测试性",
            "unit",
        ...
        case(
            "UT-EPS-DEPENDENCY",
            "Ghostscript 缺失时 QVImageLoader 必须返回含安装/路径提示的错误，image 为空，且禁止 Qt fallback 读取内嵌 placement preview。",
            "功能正确性/可测试性",
            "unit",
        ...
        case(
            "IT-EPS-SETTINGS",
            "Settings 当前三分类模型必须移除过时 Formats 表，同时保留应用的 EPS 扩展注册表。",
            "功能正确性/可测试性",
            "integration",
            "验证设置 UI 迁移后没有悬空的旧 Formats 页面契约。",
            ["fovelle_tests 已编译。", "Cocoa dialog 可构造。"],
            ["生产 QVOptionsDialog 的 categoryTabs 与对象树。"],
            ["构造 QVOptionsDialog。", "读取 categoryTabs 与旧 formats 对象。"],
            ["当前 General/Shortcuts/Mouse 三分类存在，Formats 和 formatsTable 不存在，EPS 注册表仍可查询。"],
            ["对话框销毁且不持久化设置。"],
            "tests/tst_qviewtests.cpp::FeatureTests::testSettingsFormatsPaneIsRemoved",
