...
def build_specification(repo: Path, generated_at: str) -> dict:
    cases = [
        ...
        case(
            ...
            "EPS 必须以受限 Ghostscript 子进程解释权威 PostScript，用 -dEPSCrop 生成 PDF，再由 CoreGraphics 按请求尺寸栅格化；不得以内嵌预览代替正文。",
            "精益完整性/可测试性",
            "static",
            "验证外部解释器、PDF 栅格化和资源安全边界同时存在。",
        ...
        case(
            ...
            "QVImageLoader 必须复用 native Result 且不增加 suffix 分支；QVImageCore 必须跳过 EPS 的 QMovie 探测，防止内嵌预览延迟覆盖。",
            "精益完整性/功能正确性",
            "static",
            "验证 EPS 解码结果能沿现有异步 loader 传递。",
            ["QVImageLoader 与 QVCocoaFunctions 源码可读。"],
            ["readImageWithImageIO、nativeResult.image、EPS UTI 和 QMovie 启动条件。"],
            ["检查 loader 调用 native bridge。", "检查 loader 未按 eps suffix 复制分支。", "检查 image core 按 EPS UTI 禁用 movie probe。"],
            ["EPS 共用 Result/缓存/请求语义，静态权威渲染在延迟事件后仍保留。"],
            ["不执行图像解码。"],
            "tests/eps_quality_static.py",
            "reports/evidence/eps_static.json",
        ),
        ...
        case(
            "UT-EPS-RENDER",
            "DOS EPS 样例或 deterministic vector EPS 必须从权威 PostScript 渲染为 2048px QImage，逻辑尺寸保持 BoundingBox，类型为 com.adobe.encapsulated-postscript。",
            "功能正确性",
            "unit",
            ...
            ["以 2048px hint 调用 readImageWithImageIO。", "检查 UTI、errorString、BoundingBox 逻辑尺寸、2048px raster 和像素分布/方向。"],
            ["raster 最大边为 2048 且不同于 logical size；样例方向正确，fallback 的已知黑白像素正确。"],
        ...
        case(
            ...
            ["请求完成，raster 最大边为 1920、logical size 独立保留且无 errorData。"],
            ["loader 和临时文件释放。"],
        ...
        case(
            "UT-EPS-STATIC",
            "QVImageCore 处理 EPS 后等待 1100ms，QMovie 必须保持 NotRunning，pixmap 尺寸不得从权威 raster 回退为 placement preview。",
            "功能正确性/可测试性",
            "unit",
            ...
            ["movie 始终不运行，pixmap 始终保留大于 120px 的权威 raster 尺寸。"],
        ...
        case(
            "SYS-EPS-OPEN",
            "真实 Fovelle.app 打开 EPS 后必须在 2 秒观察窗末尾保留大于 120px 的 item geometry，且无 Ghostscript/unsupported-format 错误。",
            "功能正确性/可测试性",
            "system",
            ...
            ["全部运行的最终非零 geometry 最大边 >120，无 renderer/unsupported-format 错误。"],
...
def build_quality_report(
    ...
    dimensions = [
        {
            ...
            "assessment": "复用现有 native bridge、Result、异步 loader 和 Settings registry；EPS 特有部分只负责有界 Ghostscript→PDF、CoreGraphics raster 和静态文档保护。",
        },
        {
            "id": "CQ-EPS-FUNCTION",
    ...
    return {
        ...
        "scope": "EPS/EPSF/EPSI authoritative PostScript rendering, format registration, static-document retention, dependency documentation, tests, and audit artifacts",
