...
macdeployqt Fovelle.app

IMF_DIR=Fovelle.app/Contents/PlugIns/imageformats
...
rm -f Fovelle.app/Contents/PlugIns/tls/libqopensslbackend.dylib

echo "Running codesign"
if [[ "$APPLE_NOTARIZE_REQUESTED" == "true" ]]; then
    APP_IDENTIFIER=$(/usr/libexec/PlistBuddy -c "Print CFBundleIdentifier" "Fovelle.app/Contents/Info.plist")
    codesign --sign "$APPLE_DEVID_APP_CERT_NAME" --deep --force --options runtime --timestamp "Fovelle.app"
else
    codesign --sign - --deep --force "Fovelle.app"
fi

echo "Creating disk image"
BUILD_NAME=Fovelle-$1
DMG_FILENAME=$BUILD_NAME-macOS$2.dmg
hdiutil create -srcfolder "Fovelle.app" -volname "$BUILD_NAME" -format UDSB "Fovelle.sparsebundle"
hdiutil convert "Fovelle.sparsebundle" -format ULFO -o "$DMG_FILENAME"
rm -r Fovelle.sparsebundle
if [[ "$APPLE_NOTARIZE_REQUESTED" == "true" ]]; then
